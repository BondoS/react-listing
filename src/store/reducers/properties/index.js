...
export const getAllRoomsByHotelId = id => async (dispatch, getState) => {
  const { rooms } = getState();
  const resultRooms = [];
  // and typeof rooms.list(id) !== 'undefined'
  console.log('typeof rooms.list(id)', typeof rooms.list[id]);
  console.log(
    'typeof rooms.list(id) !== "undefined"',
    typeof rooms.list[id] !== 'undefined'
  );
  console.log('Object.keys(rooms.list)', Object.keys(rooms.list));
  console.log(
    'Object.keys(rooms.list).filter(room => room.hotelId === id)',
    Object.keys(rooms.list).filter(room => room.hotelId === id)
  );
  console.log(
    'reduce',
    Object.keys(rooms.list)
      .filter(room => room.hotelId === id)
      .reduce(
        (acc, curr) => ({
          [curr]: rooms.list[curr]
        }),
        {}
      )
  );
  if (rooms.lastFetch) {
    return Object.keys(rooms.list)
      .filter(room => room.hotelId === id)
      .reduce(
        (acc, curr) => ({
          [curr]: rooms.list[curr]
